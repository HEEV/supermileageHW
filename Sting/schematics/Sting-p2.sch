EESchema Schematic File Version 4
LIBS:StingSchematics-cache
EELAYER 26 0
EELAYER END
$Descr USLetter 8500 11000 portrait
encoding utf-8
Sheet 3 5
Title "Sting - Gasoline Prototype"
Date "2016-05-25"
Rev "1.5"
Comp "Cedarville University"
Comment1 "Samuel Ellicott"
Comment2 "Throttle"
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L power:+12V #PWR?
U 1 1 57A22BC1
P 1400 900
F 0 "#PWR?" H 1400 750 50  0001 C CNN
F 1 "+12V" H 1400 1040 50  0000 C CNN
F 2 "" H 1400 900 50  0000 C CNN
F 3 "" H 1400 900 50  0000 C CNN
	1    1400 900 
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 57A22C57
P 7100 8750
F 0 "#PWR?" H 7100 8500 50  0001 C CNN
F 1 "GND" H 7100 8600 50  0000 C CNN
F 2 "" H 7100 8750 50  0000 C CNN
F 3 "" H 7100 8750 50  0000 C CNN
	1    7100 8750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 57A22C69
P 7100 850
F 0 "#PWR?" H 7100 600 50  0001 C CNN
F 1 "GND" H 7100 700 50  0000 C CNN
F 2 "" H 7100 850 50  0000 C CNN
F 3 "" H 7100 850 50  0000 C CNN
	1    7100 850 
	-1   0    0    1   
$EndComp
$Comp
L Supermileage:Servo M?
U 1 1 57A231A6
P 6300 4600
F 0 "M?" H 6300 4700 60  0001 C CNN
F 1 "Servo" H 6300 4400 60  0000 C CNN
F 2 "" H 6300 4600 60  0000 C CNN
F 3 "" H 6300 4600 60  0000 C CNN
	1    6300 4600
	1    0    0    -1  
$EndComp
$Sheet
S 3700 4250 1300 600 
U 57A2438B
F0 "Arduino" 60
F1 "Arduino.sch" 60
F2 "12v" I L 3700 4350 60 
F3 "GND" I R 5000 4350 60 
F4 "Throttle_5v" O L 3700 4500 60 
F5 "Throttle_gnd" O L 3700 4600 60 
F6 "Throttle_sig" I L 3700 4700 60 
F7 "Servo_5v" O R 5000 4500 60 
F8 "Servo_gnd" O R 5000 4600 60 
F9 "Servo_ctl" O R 5000 4700 60 
$EndSheet
$Comp
L Supermileage:Throttle U?
U 1 1 57A269DC
P 2700 4650
F 0 "U?" H 2350 4650 60  0001 C CNN
F 1 "Throttle" H 2500 4400 60  0000 C CNN
F 2 "" H 2700 4750 60  0000 C CNN
F 3 "" H 2700 4750 60  0000 C CNN
	1    2700 4650
	1    0    0    -1  
$EndComp
NoConn ~ 2900 4800
$Comp
L power:+12V #PWR?
U 1 1 5868B554
P 1400 8750
F 0 "#PWR?" H 1400 8600 50  0001 C CNN
F 1 "+12V" H 1400 8890 50  0000 C CNN
F 2 "" H 1400 8750 50  0000 C CNN
F 3 "" H 1400 8750 50  0000 C CNN
	1    1400 8750
	-1   0    0    1   
$EndComp
$Comp
L Supermileage:CB CB4
U 1 1 58697D6B
P 1750 2050
F 0 "CB4" H 1750 2250 60  0000 C CNN
F 1 "5A" H 1750 1950 60  0000 C CNN
F 2 "" H 1750 2050 60  0000 C CNN
F 3 "" H 1750 2050 60  0000 C CNN
	1    1750 2050
	1    0    0    -1  
$EndComp
$Comp
L Supermileage:CB CB5
U 1 1 58697D72
P 1750 2500
F 0 "CB5" H 1750 2700 60  0000 C CNN
F 1 "5A" H 1750 2400 60  0000 C CNN
F 2 "" H 1750 2500 60  0000 C CNN
F 3 "" H 1750 2500 60  0000 C CNN
	1    1750 2500
	1    0    0    -1  
$EndComp
$Comp
L Supermileage:SW_SPST SW12
U 1 1 58697D79
P 2450 2050
F 0 "SW12" H 2450 2250 60  0000 C CNN
F 1 "Computer" H 2450 1950 60  0000 C CNN
F 2 "" H 2450 2050 60  0000 C CNN
F 3 "" H 2450 2050 60  0000 C CNN
	1    2450 2050
	1    0    0    -1  
$EndComp
$Sheet
S 4650 1950 950  1900
U 58697D86
F0 "CM0711" 60
F1 "Computer.sch" 60
F2 "Wake" I L 4650 2050 60 
F3 "Power" I L 4650 2500 60 
F4 "GND" U R 5600 2050 60 
F5 "E_Start" O L 4650 2650 60 
F6 "E_Stop" O L 4650 2800 60 
F7 "Horn" O L 4650 2950 60 
F8 "CANH" B L 4650 3400 60 
F9 "CANL" B L 4650 3550 60 
F10 "CAN_GND" U L 4650 3700 60 
F11 "Arm_Status" I L 4650 3100 60 
$EndSheet
Text Label 3350 2050 2    60   ~ 0
Wake
Text Label 3350 2500 2    60   ~ 0
Computer_Power
Text GLabel 4600 2650 0    60   Output ~ 0
Computer_Start
Text GLabel 4600 2800 0    60   Output ~ 0
Computer_Stop
Text GLabel 4600 2950 0    60   Output ~ 0
Computer_Horn
Text GLabel 4600 3100 0    60   Input ~ 0
Arm_Status
Text GLabel 4600 3550 0    60   BiDi ~ 0
CANL
Text GLabel 4600 3400 0    60   BiDi ~ 0
CANH
Text GLabel 4600 3700 0    60   UnSpc ~ 0
CAN_GND
Text GLabel 2250 5200 2    60   UnSpc ~ 0
CAN_PWR
Wire Wire Line
	1400 900  1400 2050
Wire Wire Line
	7100 850  7100 1550
Wire Wire Line
	5900 4500 5000 4500
Wire Wire Line
	5900 4600 5000 4600
Wire Wire Line
	5900 4700 5000 4700
Wire Wire Line
	5000 4350 7100 4350
Connection ~ 7100 4350
Wire Wire Line
	2900 4500 3700 4500
Wire Wire Line
	2900 4600 3700 4600
Wire Wire Line
	2900 4700 3700 4700
Wire Wire Line
	1600 2050 1400 2050
Wire Wire Line
	1600 2500 1400 2500
Wire Wire Line
	1900 2050 2000 2050
Wire Wire Line
	2650 2050 4650 2050
Wire Wire Line
	5600 2050 7100 2050
Wire Wire Line
	1900 2500 2000 2500
Connection ~ 1400 2500
Connection ~ 1400 2050
Connection ~ 7100 2050
Wire Wire Line
	3700 4350 2000 4350
Wire Wire Line
	2000 2500 2000 4350
Connection ~ 2000 2500
Wire Wire Line
	4600 2650 4650 2650
Wire Wire Line
	4600 2800 4650 2800
Wire Wire Line
	4600 2950 4650 2950
Wire Wire Line
	4600 3100 4650 3100
Wire Wire Line
	4650 3250 4600 3250
Wire Wire Line
	4650 3400 4600 3400
Wire Wire Line
	4600 3550 4650 3550
Wire Wire Line
	2000 5200 2250 5200
Connection ~ 2000 4350
Text GLabel 4600 3250 0    60   Input ~ 0
System_V
Wire Wire Line
	4650 3700 4600 3700
Wire Wire Line
	2250 1550 2000 1550
Wire Wire Line
	2000 1050 2000 1550
Connection ~ 2000 2050
$Comp
L StingSchematics-rescue:Electromagnetic_Actor L1
U 1 1 5AB5A7EA
P 5075 1550
F 0 "L1" V 5250 1575 50  0000 L CNN
F 1 "Latch Solenoid" V 4900 1350 50  0000 L CNN
F 2 "" V 5050 1650 50  0001 C CNN
F 3 "" V 5050 1650 50  0001 C CNN
	1    5075 1550
	0    1    1    0   
$EndComp
Wire Wire Line
	2650 1550 2850 1550
Wire Wire Line
	5275 1550 7100 1550
Connection ~ 7100 1550
$Comp
L relays_switches:SW_Push SW10
U 1 1 5AB5B029
P 2450 1050
F 0 "SW10" H 2675 1225 60  0000 C CNN
F 1 "Canopy" H 2450 950 60  0000 C CNN
F 2 "" H 2450 1050 60  0000 C CNN
F 3 "" H 2450 1050 60  0000 C CNN
	1    2450 1050
	1    0    0    -1  
$EndComp
$Comp
L relays_switches:SW_Push SW11
U 1 1 5AB5D23C
P 2450 1550
F 0 "SW11" H 2650 1725 60  0000 C CNN
F 1 "Canopy" H 2450 1450 60  0000 C CNN
F 2 "" H 2450 1550 60  0000 C CNN
F 3 "" H 2450 1550 60  0000 C CNN
	1    2450 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	2000 1050 2250 1050
Connection ~ 2000 1550
Wire Wire Line
	2650 1050 2850 1050
Wire Wire Line
	2850 1050 2850 1550
Connection ~ 2850 1550
Text Label 3125 1550 0    60   ~ 0
Canopy_Open
Wire Wire Line
	7100 4350 7100 8750
Wire Wire Line
	1400 2500 1400 8750
Wire Wire Line
	1400 2050 1400 2500
Wire Wire Line
	7100 2050 7100 4350
Wire Wire Line
	2000 2500 4650 2500
Wire Wire Line
	2000 4350 2000 5200
Wire Wire Line
	2000 2050 2250 2050
Wire Wire Line
	7100 1550 7100 2050
Wire Wire Line
	2000 1550 2000 2050
Wire Wire Line
	2850 1550 4975 1550
$EndSCHEMATC
